...
async function downloadAndUploadMedia(
  ...
  console.log(
    `📥 Baixando ${mediaType} da Evolution API para mensagem:`,
    messageId,
  );
  ...
  try {
    ...
    console.log(
      `✅ Resposta da Evolution API recebida, tamanho do base64:`,
    ...
    if (!data.base64) {
      console.error(
        "❌ Base64 não encontrado na resposta:",
    ...
    console.log(
      `📤 Fazendo upload para Storage: ${fileName}, tamanho: ${binaryData.length} bytes, tipo: ${mimetype}`,
    );
    ...
    const publicUrl = withSupabasePublicOrigin(urlData.publicUrl);
...
serve(async (req) => {
  ...
  // Fail closed: the target never accepts an unsigned Evolution webhook.
  ...
  try {
    ...
    if (event === "connection.update" || event === "CONNECTION_UPDATE") {
      console.log(
        `🔄 Processando atualização de conexão para instância: ${instance}`,
      );
      ...
      if (!state || typeof state !== "string" || state.trim() === "") {
        console.log(
          "⏭️ Estado vazio ou nulo — ignorando update (mantendo status atual)",
        );
        return new Response(
      ...
      if (normalizedState === "connecting") {
        console.log(
          '⏭️ Estado transitório "connecting" — ignorando para proteger status atual',
        );
        return new Response(
          JSON.stringify({
            success: true,
            message: "Estado transitório ignorado",
          }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 200,
      ...
      if (newStatus === "CONNECTED") {
        ...
        normalizedState === "close" || normalizedState === "disconnected"
      ) {
        // DOUBLE-CHECK: Confirmar desconexão com Evolution API antes de marcar
        console.log(
          '🔍 Estado "close/disconnected" recebido — fazendo double-check na Evolution API...',
        );
        ...
        try {
          const checkResponse = await fetch(
            `${evolutionApiUrl}/instance/connectionState/${instance}`,
            {
              method: "GET",
              headers: {
          ...
          if (checkResponse.ok) {
            ...
            const realState = checkData.instance?.state || checkData.state ||
              "";
            console.log(
              "📊 Double-check: estado real na Evolution API:",
              realState,
            );

            if (isConnectedState(realState)) {
              console.log(
                "✅ Double-check: instância AINDA ESTÁ CONECTADA! Ignorando webhook de desconexão (falso positivo).",
              );
              confirmedDisconnected = false;
            } else {
              console.log(
                "❌ Double-check: desconexão confirmada. Estado real:",
                realState,
              );
            }
          } else {
            console.warn(
              "⚠️ Double-check: Evolution API retornou erro",
              checkResponse.status,
              "— assumindo desconectado",
            );
          }
        } catch (checkError) {
          console.warn(
            "⚠️ Double-check: erro ao contactar Evolution API:",
            checkError,
            "— assumindo desconectado",
          );
        }
        ...
        if (!confirmedDisconnected) {
          return new Response(
            JSON.stringify({
              success: true,
              message: "Falso positivo — conexão ainda ativa",
            }),
            {
              headers: { ...corsHeaders, "Content-Type": "application/json" },
              status: 200,
        ...
        console.log(
          "🔄 Disconexão confirmada — tentando auto-reconexão em background...",
        );
        ...
        const reconnectAttempt = async () => {
          try {
            ...
            for (let attempt = 1; attempt <= 3; attempt++) {
              console.log(
                `🔄 Auto-reconexão tentativa ${attempt}/3 para ${instance}...`,
              );

              const restartResponse = await fetch(
                `${evoUrl}/instance/restart/${instance}`,
                {
                  method: "POST",
                  headers: {
                    "Content-Type": "application/json",
              ...
              if (restartResponse.ok) {
                ...
                if (stateResponse.ok) {
                  ...
                  if (isConnectedState(currentState)) {
                    console.log(
                      `✅ Auto-reconexão bem-sucedida na tentativa ${attempt}!`,
                    );
                    await supabase
                      .from("whatsapp_instances")
                      .update({
              ...
              if (attempt < 3) {
                console.log(
                  `⏳ Tentativa ${attempt} falhou — aguardando 10s antes da próxima...`,
                );
            ...
            console.log(
              "❌ Auto-reconexão falhou após 3 tentativas — marcando como DISCONNECTED",
            );
            await supabase
              .from("whatsapp_instances")
              .update({
        ...
        reconnectAttempt().catch((err) =>
          console.error("❌ Falha na auto-reconexão background:", err)
        );
        ...
        return new Response(
          JSON.stringify({
            success: true,
            message: "Auto-reconexão iniciada em background",
          }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 200,
        ...
        console.log(
          `⏭️ Estado não reconhecido "${normalizedState}" — ignorando para proteger status atual`,
        );
        return new Response(
          JSON.stringify({
            success: true,
            message: `Estado não reconhecido ignorado: ${normalizedState}`,
          }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 200,
        ...
        try {
          ...
          if (!rawPhone) {
            try {
              ...
              console.warn(
                "⚠️ Falha ao consultar fetchInstances para phone_number:",
                fetchErr,
              );
            }
          }
      ...
      if (newStatus !== "CONNECTED") {
        ...
        if (updateError) {
          ...
          console.log(
            `✅ Status atualizado para ${newStatus}: ${instance} (com proteção CONNECTED)`,
          );
        }
      } else {
        const { error: updateError } = await supabase
      ...
      return new Response(
        JSON.stringify({
          success: true,
          message: `Status atualizado: ${newStatus}`,
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 200,
    ...
    if (event === "presence.update" || event === "PRESENCE_UPDATE") {
      ...
      if (!remoteJid || !lastKnownPresence) {
        return new Response(
          JSON.stringify({
            success: true,
            message: "Presence sem dados suficientes",
          }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 200,
      ...
      if (remoteJid.endsWith("@g.us")) {
        return new Response(
          JSON.stringify({
            success: true,
            message: "Presence de grupo ignorado",
          }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 200,
      ...
      if (!presencePhone || presencePhone.length < 8) {
        return new Response(
          JSON.stringify({
            success: true,
            message: "Presence sem phone valido",
          }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 200,
      ...
      const isOnline = lastKnownPresence === "available" ||
        lastKnownPresence === "composing" || lastKnownPresence === "recording";
      ...
      console.log(
        `✅ Presenca atualizada: ${presencePhone} -> ${lastKnownPresence} (online=${isOnline})`,
      );
    ...
    if (event === "messages.update" || event === "MESSAGES_UPDATE") {
      ...
      const mapStatus = (raw: any): string | null => {
        ...
        if (s === "DELIVERY_ACK" || s === "DELIVERED" || s === "3") {
          return "DELIVERED";
        }
      ...
      const rank = (
        s: string | null,
      ) => (s === "READ" ? 3 : s === "DELIVERED" ? 2 : s === "SENT" ? 1 : 0);
      ...
      for (const upd of updates) {
        ...
        if (updErr) {
          console.error(
            `❌ Erro ao atualizar status_entrega em ${table}:`,
            updErr,
          );
        } else {
          console.log(
            `✅ ${table}.${existingRow.id} status_entrega -> ${newStatus}`,
          );
        }
      }
      ...
      return new Response(
        JSON.stringify({
          success: true,
          message: "messages.update processado",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 200,
    ...
    if (event !== "messages.upsert" && event !== "MESSAGES_UPSERT") {
      ...
      return new Response(
        JSON.stringify({
          success: true,
          message: `Evento ${event} não processado`,
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 200,
    ...
    const { data: instanceData, error: instanceError } = await supabase
      .from("whatsapp_instances")
      .select(
        "id, user_id, phone_number, organization_id, accepts_leads, instance_name",
      )
      .eq("instance_name", instance)
      .maybeSingle();
    ...
    if (!instanceData) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error:
            "Instância WhatsApp não encontrada. Por favor, reconecte o WhatsApp.",
    ...
    if (!organizationId) {
      console.error(
        "❌ Organization não encontrada para a instância:",
        instance,
      );
      return new Response(
        JSON.stringify({
          success: false,
          error:
            "Organização não encontrada para esta instância. Por favor, reconecte o WhatsApp.",
          instance: instance,
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const saveWebhookLog = async (status: string, errorMessage?: string) => {
      try {
        ...
        if (data?.key?.fromMe !== undefined) {
          logData.direction = data.key.fromMe ? "SENT" : "RECEIVED";
        }
    ...
    console.log("📦 Estrutura de evento validada", {
      event,
    ...
    console.log("🔑 Metadados da mensagem", {
      hasMessageId: Boolean(messageKey?.id),
      fromMe: Boolean(messageKey?.fromMe),
      hasMessageInfo: Boolean(messageInfo),
    });
    ...
    if (remoteJid.endsWith("@g.us")) {
      console.log(
        "👥 Mensagem de grupo recebida — salvando em mensagens_grupo:",
        remoteJid,
      );
      ...
      const collectMentioned = (info: any): string[] => {
        ...
        const tryGet = (path: any) => {
          ...
          if (Array.isArray(arr)) {
            candidates.push(...arr.filter((x: any) => typeof x === "string"));
          }
        };
      ...
      const findQuoted = (
        ...
        | { stanzaId: string | null; participant: string | null; raw: any }
        | null => {
      ...
      const summarizeQuoted = (
        ...
        if (q.extendedTextMessage?.text) {
          return { body: q.extendedTextMessage.text, mediaType: null };
        }
        if (q.imageMessage) {
          return {
            body: q.imageMessage.caption || "[Imagem]",
            mediaType: "image",
          };
        }
        if (q.videoMessage) {
          return {
            body: q.videoMessage.caption || "[Vídeo]",
            mediaType: "video",
          };
        }
        ...
        if (q.documentMessage) {
          return {
            body: q.documentMessage.fileName || "[Documento]",
            mediaType: "document",
          };
        }
        if (q.stickerMessage) {
          return { body: "[Figurinha]", mediaType: "sticker" };
        }
      ...
      if (
        groupMediaUrl && groupMediaType && groupMsgId && serverUrl && apiKey
      ) {
        try {
          ...
          console.warn(
            "⚠️ Falha ao persistir midia de grupo no Storage — mantendo URL temporario:",
            mediaErr,
          );
        }
      }
      ...
      const finalMetadata: any = groupMediaMetadata
        ? { ...groupMediaMetadata }
        : {};
      ...
      try {
        const { error: insertError } = await supabase.from("mensagens_grupo")
          .insert({
            organization_id: organizationId,
            ...
            media_metadata: Object.keys(finalMetadata).length > 0
              ? finalMetadata
              : null,
        ...
        if (insertError) {
          ...
          if (code === "23505") {
            console.log(
              "ℹ️ Mensagem de grupo duplicada (reprocessamento) — ignorada com sucesso",
            );
          } else {
            console.error("❌ Erro ao salvar mensagem de grupo:", insertError);
            await saveWebhookLog(
              "error",
              `mensagens_grupo insert: ${insertError.message}`,
            );
          }
        } else {
      ...
      return new Response(
        JSON.stringify({
          success: true,
          message: "Mensagem de grupo processada",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 200,
      ...
      if (incomingMessageId) {
        ...
        if (existingMsg) {
          ...
          return new Response(
            JSON.stringify({
              success: true,
              message: "Mensagem propria ja registrada",
            }),
            {
              headers: { ...corsHeaders, "Content-Type": "application/json" },
              status: 200,
        ...
        console.log(
          "📲 Mensagem fromMe nova (enviada pelo celular) - sera salva como SAIDA",
        );
      } else {
    ...
    if (messageInfo.conversation) {
      ...
      messageContent = `[Documento] ${
        messageInfo.documentMessage.fileName || ""
      }`;
    ...
    if (existingLead) {
      ...
      if (!existingLead.funnel_id) {
        console.log(
          "🔄 Lead existente sem funil, aplicando mapeamento padrão de funil para WhatsApp...",
        );
        ...
        if (funnelIds.length > 0) {
          ...
          if (funnelMapping) {
            console.log(
              "✅ Mapeamento encontrado para lead existente:",
              funnelMapping,
            );
            ...
            console.log(
              "⚠️ Nenhum mapeamento encontrado para lead existente, usando funil padrão",
            );
            ...
            const defaultFunnel = defaultFunnels && defaultFunnels.length > 0
              ? defaultFunnels[0]
              : null;
        ...
        if (funnelId && funnelStageId) {
          console.log("✅ Atualizando funil do lead existente:", {
            funnelId,
            funnelStageId,
          });
          await supabase
          ...
          console.log(
            "⚠️ Não foi possível determinar funil/etapa para o lead existente",
          );
        }
      }
      ...
      if (instanceData.accepts_leads === false) {
        console.log(
          "Canal configurado apenas para conversas. Novo contato nao sera criado como lead:",
          phoneNumber,
        );
        await saveWebhookLog(
          "ignored",
          "Canal WhatsApp configurado apenas para conversas",
        );
        return new Response(
          JSON.stringify({
            success: true,
      ...
      if (createLeadError) {
        ...
        console.error("❌ Lead data tentado:", {
          phoneNumber,
          newLeadName,
          organizationId,
        });
      ...
      try {
        ...
        console.warn(
          "⚠️ [webhook] erro ao aplicar tag de anuncio (nao bloqueia):",
          tagErr,
        );
      }
      ...
      const profilePicturePromise = supabase.functions.invoke(
    ...
    console.log(
      `  - originalMediaUrl: ${originalMediaUrl ? "PRESENTE" : "AUSENTE"}`,
    );
    ...
    if (originalMediaUrl && mediaType && leadId && serverUrl && apiKey) {
      console.log(
        `📥 ✅ TODAS CONDIÇÕES OK - Processando mídia do tipo ${mediaType}...`,
      );
      try {
      ...
      console.log(
        "⚠️ ⚠️ ⚠️ CONDIÇÕES NÃO ATENDIDAS - mídia não será processada",
      );
    ...
    if (instanceId) {
      ...
      if (lcmInsertError) {
        ...
        if (code === "23505") {
          ...
          if (lcmUpdateError) {
            console.warn(
              "⚠️ Falha ao UPDATE last_message_at em lead_channel_memberships:",
              lcmUpdateError,
            );
          }
        } else {
          console.warn(
            "⚠️ Falha ao INSERT em lead_channel_memberships (nao bloqueia):",
            lcmInsertError,
          );
        }
      }
    ...
    if (saveMessageError) {
      ...
      console.error("❌ Mensagem data tentada:", {
        leadId,
        messageContent,
        messageId,
      });
