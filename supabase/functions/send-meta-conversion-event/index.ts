...
serve(async (req) => {
  ...
  try {
    const { lead_id, event_name = "Purchase", value, currency = "BRL" } =
      await req.json() as ConversionEventRequest;

    if (!lead_id) {
      throw new RequestAuthorizationError(400, "lead_id is required");
    }
    ...
    const { data: lead, error: leadError } = await supabase
      .from("leads")
      .select(
        "id, organization_id, email, telefone_lead, nome_lead, valor, funnel_id",
      )
      .eq("id", lead_id)
      .single();
    ...
    if (!isInternalServiceRoleRequest(req)) {
      const { token } = await requireOrganizationMember(
        req,
        supabase,
      ...
      const { data: canAccessLead, error: accessError } = await userClient
        .rpc("user_can_access_lead", { p_lead_id: lead_id });

      if (accessError) {
        console.error("[Meta CAPI] Lead access check failed:", accessError);
        throw new RequestAuthorizationError(
          500,
          "Could not validate lead access",
        );
      }
      if (!canAccessLead) {
        throw new RequestAuthorizationError(403, "Access denied to this lead");
      }
    }
    ...
    if (pixelError || !pixelConfig) {
      console.log(
        `[Meta CAPI] No active pixel configuration found for organization ${lead.organization_id}`,
      );
      return new Response(
        JSON.stringify({
          success: false,
          message: "No pixel configuration found for this organization",
        }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }
    ...
    console.log(
      `[Meta CAPI] Sending event to Pixel ${pixelConfig.pixel_id}:`,
