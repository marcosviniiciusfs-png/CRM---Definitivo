...
serve(async (req) => {
  ...
  try {
    ...
    const { email, password, name, role, organizationId, custom_role_id } =
      body;
    ...
    if (!email || !password || !name || !role || !organizationId) {
      return new Response(
        JSON.stringify({
          error:
            "Dados incompletos: email, senha, nome, cargo e organização são obrigatórios",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 400,
    ...
    if (!["owner", "admin", "member"].includes(role)) {
      return new Response(
        JSON.stringify({ error: "Papel de organização inválido" }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 400,
        },
    ...
    if (memberData.role !== "owner" && memberData.role !== "admin") {
      return new Response(
        JSON.stringify({
          error:
            "Apenas proprietários e administradores podem adicionar colaboradores",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 403,
    ...
    if (memberData.role === "admin" && role === "owner") {
      return new Response(
        JSON.stringify({
          error: "Apenas proprietários podem adicionar outro proprietário",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 403,
    ...
    if (custom_role_id) {
      ...
      if (customRoleError || !customRole) {
        return new Response(
          JSON.stringify({
            error: "Cargo personalizado inválido para esta organização",
          }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 400,
    ...
    console.log(
      `[ADD-MEMBER] Limite: ${maxCollaborators}, Total membros: ${totalCount}`,
    );
    ...
    if (
      !countError && activeCount !== null && activeCount >= maxCollaborators
    ) {
      return new Response(
        JSON.stringify({
          error:
            `Limite de ${maxCollaborators} colaboradores atingido para esta organização`,
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 400,
    ...
    if (userId) {
      ...
      if (memberError) {
        console.error(
          "[ADD-MEMBER] Erro ao adicionar membro existente:",
          memberError,
        );
        return new Response(
          JSON.stringify({
            error: "Não foi possível adicionar o colaborador à organização",
      ...
      console.log(
        "[ADD-MEMBER] Pré-cadastrando na organização (sem user_id)...",
      );
      ...
      if (preInsertError) {
        ...
        return new Response(
          JSON.stringify({
            error: "Não foi possível preparar o cadastro do colaborador",
            details: preInsertError.message,
          }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 400,
      ...
      try {
        ...
        console.log(
          "[ADD-MEMBER] Limpando pré-cadastro devido a erro no auth...",
        );
        await supabaseAdmin
          .from("organization_members")
          .delete()
        ...
        if (
          errorMsg.includes("subscriptions") && errorMsg.includes("user_id")
        ) {
          console.error(
            "[ADD-MEMBER] Erro de subscription detectado - auth hook pode estar mal configurado",
          );
          return new Response(
            JSON.stringify({
              error:
                "Erro de configuração: Auth Hook está tentando criar subscription incorretamente. Contate o administrador.",
              phase: "createUser",
              details: errorMsg,
            }),
            {
        ...
        return new Response(
          JSON.stringify({
            error: errorMsg || "Não foi possível criar o usuário",
            phase: "createUser",
          }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      if (!newUser?.user) {
        ...
        return new Response(
          JSON.stringify({
            error: "Não foi possível criar o usuário - resposta vazia",
            phase: "createUser",
          }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      if (subscriptionError) {
        console.warn(
          "[ADD-MEMBER] Aviso ao criar subscription:",
      ...
      console.log(
        "[ADD-MEMBER] Atualizando organization_members com user_id...",
      );
      ...
      if (updateMemberError) {
        console.error(
          "[ADD-MEMBER] Erro ao atualizar membro:",
          updateMemberError,
        );
      } else {
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: `${name} foi adicionado à organização com sucesso`,
        userId,
      }),
      {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
