...
const getLeadTypeName = (leadType: string): string => {
  if (!leadType) return "";
  if (
    leadType.includes("messaging") ||
    leadType.includes("total_messaging_connection")
  ) return "Mensagem";
  if (
    leadType === "lead" || leadType === "leadgen_grouped" ||
    leadType.includes("lead_grouped")
  ) return "Formulário";
  ...
  if (
    leadType.includes("offsite_conversion.custom.") ||
    leadType.includes("omni_custom")
...
const calculateLeadsFromActions = (
  ...
  const actionSummary = actions.slice(0, 15).map((a: any) =>
    `${a.action_type}:${a.value}`
  ).join(", ");
  console.log(
    `    [ACTIONS] Objetivo=${
      objective || "N/A"
    }, Disponíveis: ${actionSummary}`,
  );
  ...
  if (objective) {
    ...
    if (expectedTypes) {
      for (const expectedType of expectedTypes) {
        for (const action of actions) {
          if (
            ...
            action.action_type.startsWith(expectedType)
          ) {
            const value = parseInt(action.value || "0", 10);
            if (value > 0) {
              console.log(
                `    [MATCH] Por objetivo ${objective}: ${action.action_type}=${value}`,
              );
      ...
      console.log(
        `    [INFO] Nenhum match para objetivo ${objective}, usando fallback...`,
      );
    }
  }
  ...
  for (const action of actions) {
    if (
      action.action_type.startsWith("offsite_conversion.custom.") ||
...
const fetchAllPages = async (baseUrl: string): Promise<any[]> => {
  ...
  while (url && pageCount < maxPages) {
    try {
      ...
      if (jsonData.error) {
        console.error(
          `[PAGINATION] Erro na página ${pageCount + 1}:`,
      ...
      if (jsonData.data && jsonData.data.length > 0) {
        ...
        console.log(
          `[PAGINATION] Página ${
            pageCount + 1
          }: ${jsonData.data.length} registros`,
        );
      }
      ...
      console.error(
        `[PAGINATION] Erro ao buscar página ${pageCount + 1}:`,
        err,
      );
      break;
    }
  ...
  console.log(
    `[PAGINATION] Total: ${allData.length} registros em ${pageCount} página(s)`,
  );
  return allData;
};
...
const fetchCampaignIdsForPage = async (
  ...
  try {
    ...
    console.warn(
      "[ADS] Erro ao filtrar campanhas por criativos da página:",
      adsError,
    );
  }
  ...
  try {
    ...
    console.warn(
      "[ADS] Erro ao filtrar campanhas por conjuntos da página:",
      adsetsError,
    );
  }

  console.log(
    `[ADS] Campanhas vinculadas à página ${pageId}: ${pageCampaignIds.size}`,
  );
  return pageCampaignIds;
};

const fetchBusinessAdAccounts = async (
  businessId: string,
  accessToken: string,
): Promise<AdAccount[]> => {
  ...
  for (const edge of ["owned_ad_accounts", "client_ad_accounts"]) {
    try {
      const accountsUrl =
        `https://graph.facebook.com/v21.0/${businessId}/${edge}?fields=id,name,account_status&limit=50&access_token=${accessToken}`;
      const accounts = await fetchAllPages(accountsUrl);
      for (
        const account of normalizeAdAccounts(accounts.map((a: any) => ({
          id: a.id,
          name: a.name,
      ...
      console.warn(
        `[ADS] Erro ao buscar ${edge} da BM ${businessId}:`,
...
const discoverAccountsUsingPage = async (
  ...
  { accounts: AdAccount[]; campaignIdsByAccount: Record<string, string[]> }
> => {
  const accountMap = new Map<string, AdAccount>();
  ...
  try {
    ...
    console.warn(
      "[ADS] Erro ao listar contas do usuário para prova por página:",
      userAccountsError,
    );
  }
  ...
  try {
    ...
    for (const business of businesses) {
      for (
        const account of await fetchBusinessAdAccounts(business.id, accessToken)
      ) {
        accountMap.set(account.id, account);
      }
    }
    ...
    console.warn(
      "[ADS] Erro ao listar BMs para prova por página:",
  ...
  for (const account of accountMap.values()) {
    const campaignIds = await fetchCampaignIdsForPage(
      account.id,
      accessToken,
      pageId,
    );
  ...
  console.log(
    `[ADS] Contas comprovadas usando a página ${pageId}: ${provenAccounts.length}`,
  );
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { organization_id, start_date, end_date, ad_account_id }:
      AdsInsightsParams = await req.json();

    if (!organization_id || !start_date || !end_date) {
      throw new Error(
        "Missing required parameters: organization_id, start_date, end_date",
      );
    }
    ...
    console.log(`\n========== FETCH ADS INSIGHTS ==========`);
    console.log(
      `Org: ${organization_id}, Período: ${start_date} a ${end_date}`,
    );
    ...
    selectedAccountId = normalizeAdAccountId(
      ...
      if (Array.isArray(integration.ad_accounts)) {
        ...
        try {
          availableAccounts = normalizeAdAccounts(
            JSON.parse(integration.ad_accounts),
          );
        } catch (e) {
    ...
    const { data: secureTokens } = await supabase.rpc(
    ...
    if (
      secureTokens && secureTokens.length > 0 &&
      secureTokens[0].encrypted_access_token
    ) {
      console.log("Using secure tokens");
      access_token = await decryptMetaToken(
        secureTokens[0].encrypted_access_token,
      );
    }
    ...
    if (!access_token) {
      ...
      if (directToken?.encrypted_access_token) {
        access_token = await decryptMetaToken(
          directToken.encrypted_access_token,
        );
    ...
    if (!access_token) {
      ...
      return new Response(
        JSON.stringify({
          error:
            "Token do Facebook expirado ou não encontrado. Por favor, reconecte sua conta do Facebook nas configurações de integrações.",
          data: null,
          needsReconnect: true,
        }),
        {
    ...
    if (hasConnectedPage && access_token) {
      console.log(
        `[ADS] Modo restrito por página conectada: ${
      ...
      try {
        ...
        if (pageBusinessId) {
          ...
          const pageBusinessAccounts = await fetchBusinessAdAccounts(
            resolvedBusinessId,
            access_token,
          );
          if (pageBusinessAccounts.length > 0) {
            const businessAccountIds = new Set(
              pageBusinessAccounts.map((acc) => acc.id),
            );
            availableAccounts = pageBusinessAccounts;
            selectedAccountId =
              selectedAccountId && businessAccountIds.has(selectedAccountId)
                ? selectedAccountId
            ...
            await supabase
              ...
              .eq("id", integrationId);
            console.log(
              `[ADS] Conta selecionada restrita à BM da página: ${selectedAccountId}`,
            );
          } else {
          ...
          console.log(
            "[ADS] Página conectada não possui Business Manager visível pelo token; não usando conta pessoal automaticamente",
          );
          selectedAccountId = null;
          availableAccounts = [];
        }
        ...
        console.warn(
          "[ADS] Erro ao resolver BM da página conectada; bloqueando fallback pessoal:",
          pageBusinessError,
        );
        selectedAccountId = null;
        availableAccounts = [];
      ...
      if (!selectedAccountId && integration.page_id) {
        console.log(
          "[ADS] BM da página não trouxe conta válida; procurando contas com campanhas que usam a página conectada",
        );
        const proven = await discoverAccountsUsingPage(
          access_token,
          integration.page_id,
          availableAccounts,
        );
        ...
        if (selectedAccountId) {
          await supabase
            ...
            .eq("id", integrationId);
          console.log(
            `[ADS] Conta selecionada por prova de uso da página: ${selectedAccountId}`,
          );
        }
      }
    }
    ...
    if (!selectedAccountId && !hasConnectedPage) {
      console.log(
        "[ADS] Nenhuma conta configurada — tentando auto-descoberta...",
      );
      ...
      if (availableAccounts.length > 0) {
        ...
        try {
          ...
          if (integration.business_id) {
            console.log(
              `[ADS] Tentativa 1 — Business Manager conectado: ${integration.business_id}`,
            );
            try {
              accounts = await fetchBusinessAdAccounts(
                integration.business_id,
                access_token,
              );
              if (accounts.length > 0) {
                console.log(
                  `[ADS] Encontrou ${accounts.length} conta(s) via BM conectado`,
                );
              }
            } catch (bizErr) {
          ...
          if (accounts.length === 0) {
            ...
            const adAccountsUrl =
              `https://graph.facebook.com/v21.0/me/adaccounts?fields=id,name,account_status&limit=50&access_token=${access_token}`;
            const adAccountsResponse = await fetch(adAccountsUrl);
            const adAccountsData = await adAccountsResponse.json();

            if (adAccountsData.error) {
              console.error(
                "[ADS] Erro na auto-descoberta /me/adaccounts:",
                adAccountsData.error.message,
              );
              if (
                adAccountsData.error.code === 200 ||
                adAccountsData.error.message?.includes("permission")
              ) {
                return new Response(
                  JSON.stringify({
                    error:
                      "Sua conta do Facebook não tem permissão para acessar contas de anúncios. Reconecte sua conta autorizando o acesso a anúncios.",
                    data: null,
                    needsReconnect: true,
                  }),
                  {
              ...
              return new Response(
                JSON.stringify({
                  error:
                    `Erro ao buscar contas de anúncio: ${adAccountsData.error.message}`,
                  data: null,
                  needsReconnect: true,
                }),
                {
            ...
            accounts = normalizeAdAccounts(
              (adAccountsData.data || []).map((a: any) => ({
                id: a.id,
                name: a.name,
                status: a.account_status ?? 1,
              })),
          ...
          if (accounts.length === 0) {
            console.log(
              "[ADS] Tentativa 3 — Descobrindo Business Managers via /me/businesses",
            );
            try {
              const bizListUrl =
                `https://graph.facebook.com/v21.0/me/businesses?fields=id,name&limit=50&access_token=${access_token}`;
              ...
              if (bizListData.data && bizListData.data.length > 0) {
                console.log(
                  `[ADS] Encontrou ${bizListData.data.length} Business Manager(s)`,
                );
                for (const biz of bizListData.data) {
                  ...
                  try {
                    ...
                    if (accounts.length > 0) {
                      console.log(
                        `[ADS] Encontrou ${accounts.length} conta(s) no BM "${biz.name}" (${biz.id})`,
                      );
                      await supabase
                        .from("facebook_integrations")
                    ...
                    console.warn(
                      `[ADS] Erro ao buscar contas no BM ${biz.id}:`,
                      accErr,
                    );
                  }
                }
              ...
              console.warn(
                "[ADS] Erro ao listar Business Managers:",
                bizListErr,
              );
            }
          }
          ...
          if (accounts.length === 0) {
            return new Response(
              JSON.stringify({
                error:
                  "Nenhuma conta de anúncios encontrada. Certifique-se de que sua conta do Facebook tem acesso ao Gerenciador de Anúncios.",
                data: null,
                availableAccounts: [],
              }),
              {
          ...
          console.log(
            `[ADS] Auto-descoberta: ${accounts.length} conta(s). Usando: ${selectedAccountId}`,
          );
          ...
          return new Response(
            JSON.stringify({
              error: "Erro ao descobrir contas de anúncios. Tente novamente.",
              data: null,
            }),
            {
              headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (!resolvedBusinessId && access_token && !hasConnectedPage) {
      ...
      try {
        ...
        if (integration.page_id) {
          ...
          if (biz?.id) {
            ...
            console.log(
              `[ADS] Business descoberto pela página conectada (${
                integration.page_name || integration.page_id
              }): ${biz.name} (${biz.id})`,
            );
          }
          ...
          if (biz?.id) {
            resolvedBusinessId = biz.id;
            console.log(
              `[ADS] Business descoberto via página: ${biz.name} (${biz.id})`,
            );
          }
        }
      } catch (e) {
      ...
      if (!resolvedBusinessId) {
        try {
          const bizListUrl =
            `https://graph.facebook.com/v21.0/me/businesses?fields=id,name&limit=10&access_token=${access_token}`;
          ...
          if (bizListData.data && bizListData.data.length > 0) {
            ...
            for (const biz of bizListData.data) {
              try {
                const checkUrl =
                  `https://graph.facebook.com/v21.0/${biz.id}/owned_ad_accounts?fields=id&limit=50&access_token=${access_token}`;
                const checkResp = await fetch(checkUrl);
                const checkData = await checkResp.json();
                if (checkData.data) {
                  const found = checkData.data.find((a: any) =>
                    normalizeAdAccountId(a.id) === selectedAccountId
                  );
                  if (found) {
                    resolvedBusinessId = biz.id;
                    console.log(
                      `[ADS] Business descoberto via /me/businesses: ${biz.name} (${biz.id})`,
                    );
                    break;
                  }
                }
            ...
            if (!resolvedBusinessId) {
              ...
              console.log(
                `[ADS] Usando primeiro BM: ${bizListData.data[0].name} (${
                  bizListData.data[0].id
                })`,
              );
            }
          }
    ...
    if (resolvedBusinessId && availableAccounts.length > 0 && access_token) {
      try {
        const bmAccounts = await fetchBusinessAdAccounts(
          resolvedBusinessId,
          access_token,
        );

        if (bmAccounts.length > 0) {
          const bmAccountIds = new Set(
            bmAccounts.map((a: any) => normalizeAdAccountId(a.id)),
          );
          const filtered = availableAccounts.filter((acc) =>
            bmAccountIds.has(acc.id)
          );
          availableAccounts = filtered.length > 0 ? filtered : bmAccounts;
          console.log(
            `[ADS] Contas restritas à BM da página ${resolvedBusinessId}: ${availableAccounts.length} conta(s)`,
          );
        } else {
          console.log(
            `[ADS] BM ${resolvedBusinessId} retornou 0 contas via API — substituindo lista pela resposta da BM`,
          );
        ...
        console.warn(
          "[ADS] Erro ao filtrar contas por BM, usando lista completa:",
          filterErr,
        );
      }
    }
    ...
    if (
      selectedAccountId && availableAccounts.length === 0 && access_token &&
      !hasConnectedPage
    ) {
      try {
        const accountInfoUrl =
          `https://graph.facebook.com/v21.0/${selectedAccountId}?fields=id,name,account_status&access_token=${access_token}`;
        ...
        if (accountInfoData?.id && !accountInfoData.error) {
          ...
          console.log(
            `[ADS] Conta salva resolvida via Meta API: ${
              availableAccounts[0].name
            }`,
          );
        } else {
          console.warn(
            "[ADS] Conta salva não resolvida, tentando /me/adaccounts:",
            accountInfoData?.error?.message,
          );
          const adAccountsUrl =
            `https://graph.facebook.com/v21.0/me/adaccounts?fields=id,name,account_status&limit=50&access_token=${access_token}`;
          const adAccountsData = await (await fetch(adAccountsUrl)).json();
          const discoveredAccounts = normalizeAdAccounts(
            (adAccountsData.data || []).map((a: any) => ({
              id: a.id,
              name: a.name,
              status: a.account_status ?? 1,
          ...
          if (discoveredAccounts.length > 0) {
            ...
            const selectedFromDiscovery = availableAccounts.find((acc) =>
            ...
            if (!selectedFromDiscovery) {
              ...
              await supabase
                ...
                .eq("id", integrationId);
              console.log(
                `[ADS] Conta salva indisponível — usando conta descoberta: ${selectedAccountId}`,
              );
            }
          }
        }
        ...
        console.warn(
          "[ADS] Erro ao resolver nome/lista da conta salva:",
          accountInfoError,
        );
      }
    }
    ...
    if (hasConnectedPage && !selectedAccountId) {
      return new Response(
        JSON.stringify({
          error:
            "Nenhuma conta de anúncios vinculada à página conectada foi encontrada. As métricas não usarão contas pessoais do perfil do Facebook.",
          data: null,
    ...
    const selectedInFiltered = availableAccounts.find((acc) =>
      acc.id === selectedAccountId
    );
    if (!selectedInFiltered && availableAccounts.length > 0) {
      ...
      selectedAccountId = normalizeAdAccountId(availableAccounts[0].id);
      console.log(
        `[ADS] Conta ${oldId} não pertence à BM — auto-selecionando: ${selectedAccountId}`,
      );
      // Atualizar no banco
      await supabase
        .from("facebook_integrations")
    ...
    if (!selectedAccountId) {
      return new Response(
        JSON.stringify({
          error: "Nenhuma conta de anúncios disponível para esta integração.",
          data: null,
          selectedAccount: null,
          availableAccounts,
        }),
    ...
    console.log(
      `Conta de Anúncios: ${selectedAccountId} (${selectedAccount.name})`,
    );
    console.log(
      `Página conectada: ${integration.page_name || "sem nome"} (${
        integration.page_id || "sem page_id"
      })`,
    );

    let pageCampaignIds = provenCampaignIdsByAccount[selectedAccountId]
      ? new Set(provenCampaignIdsByAccount[selectedAccountId])
      : await fetchCampaignIdsForPage(
        selectedAccountId,
        access_token,
        integration.page_id,
      );
    ...
    if (
      hasConnectedPage && pageCampaignIds && pageCampaignIds.size === 0 &&
      availableAccounts.length > 1
    ) {
      console.log(
        "[ADS] Conta selecionada não possui campanhas da página; procurando em outras contas da BM da página",
      );
      for (const account of availableAccounts) {
        ...
        if (candidateCampaignIds && candidateCampaignIds.size > 0) {
          ...
          await supabase
            ...
            .eq("id", integrationId);
          console.log(
            `[ADS] Conta corrigida para campanhas da página conectada: ${selectedAccountId}`,
          );
          break;
        }
      }
    ...
    if (
      ...
      console.log(
        "[ADS] Nenhuma conta atual comprovou uso da página; varrendo contas acessíveis por prova de page_id",
      );
      const proven = await discoverAccountsUsingPage(
        access_token,
        integration.page_id,
      ...
      if (proven.accounts.length > 0) {
        ...
        pageCampaignIds = new Set(
          provenCampaignIdsByAccount[selectedAccountId] || [],
        );
        await supabase
          .from("facebook_integrations")
          .update({
          ...
          .eq("id", integrationId);
        console.log(
          `[ADS] Conta corrigida por campanha que usa a página: ${selectedAccountId}`,
        );
      }
    }
    ...
    if (hasConnectedPage && pageCampaignIds && pageCampaignIds.size === 0) {
      return new Response(
        JSON.stringify({
          error:
            "Nenhuma campanha vinculada à página conectada foi encontrada nesta conta de anúncios. As métricas não usarão campanhas do perfil pessoal.",
          data: null,
          selectedAccount: null,
          availableAccounts: [],
        }),
    ...
    const aggregatedInsightsUrl =
      `https://graph.facebook.com/v21.0/${selectedAccountId}/insights?` +
    ...
    const aggregatedData = pageCampaignIds
      ? rawAggregatedData.filter((record: any) =>
        pageCampaignIds.has(record.campaign_id)
      )
      : rawAggregatedData;

    if (pageCampaignIds && rawAggregatedData.length !== aggregatedData.length) {
      console.log(
        `[ADS] Filtrado pela página conectada: ${aggregatedData.length}/${rawAggregatedData.length} campanhas com insights`,
      );
    }

    if (!aggregatedData || aggregatedData.length === 0) {
      console.log(
        "Nenhum dado de insights encontrado para o período; buscando lista de campanhas da conta",
      );
      const campaigns =
        (await fetchCampaignList(selectedAccountId, access_token))
          .filter((campaign: any) =>
            !pageCampaignIds || pageCampaignIds.has(campaign.id)
          );
    ...
    const campaignIds = [
      ...new Set(aggregatedData.map((r: any) => r.campaign_id).filter(Boolean)),
    ];
    console.log(
      `\n[STEP 2] Buscando objetivos de ${campaignIds.length} campanhas...`,
    );

    const campaignObjectives: Record<
      string,
      { objective: string; optimization_goal: string }
    > = {};

    for (let i = 0; i < campaignIds.length; i += 50) {
      const batch = campaignIds.slice(i, i + 50);
      try {
        const campaignsUrl = `https://graph.facebook.com/v21.0/?ids=${
          batch.join(",")
        }&fields=objective,optimization_goal&access_token=${access_token}`;
    ...
    console.log(
      `Objetivos obtidos: ${Object.keys(campaignObjectives).length} campanhas`,
    );
    ...
    for (const record of aggregatedData) {
      ...
      const lpvAction = record.actions?.find((a: any) =>
        a.action_type === "landing_page_view"
      );
      const landingPageViews = lpvAction
        ? parseInt(lpvAction.value || "0", 10)
        : 0;

      const outboundAction = record.outbound_clicks?.find((a: any) =>
        a.action_type === "outbound_click"
      );
      const outboundClicks = outboundAction
        ? parseInt(outboundAction.value || "0", 10)
        : 0;
      ...
      const costPerLead = getLeadCostFromActions(
      ...
      campaignData[campaignKey].leadType = leadType ||
        campaignData[campaignKey].leadType;
      campaignData[campaignKey].costPerLead = costPerLead ||
        campaignData[campaignKey].costPerLead;
      campaignData[campaignKey].frequency = frequency ||
        campaignData[campaignKey].frequency;
      campaignData[campaignKey].qualityRanking = record.quality_ranking || "";
      campaignData[campaignKey].engagementRanking =
        record.engagement_rate_ranking || "";
      campaignData[campaignKey].conversionRanking =
        record.conversion_rate_ranking || "";
    ...
    const crmValidation = {
      ...
      captureRate: totalLeads > 0
        ? ((crmLeadsCount || 0) / totalLeads) * 100
        : 0,
