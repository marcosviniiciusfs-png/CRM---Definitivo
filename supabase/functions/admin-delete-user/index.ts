...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (typeof admin_password !== "string" || !admin_password.trim()) {
      return errorResp(
        "admin_password é obrigatório para confirmar a exclusão",
      );
    }
    ...
    // 2. Validar obrigatoriamente a senha do mesmo administrador da sessão.
    ...
    if (sessionError || !session?.admin_email) {
      return errorResp("Sessão administrativa inválida ou expirada", 401);
    }

    const { data: pwValid, error: passwordError } = await adminClient.rpc(
    ...
    if (orgId) {
      ...
      await adminClient.from("agent_distribution_settings").delete().eq(
        "organization_id",
        orgId,
      );
      await adminClient.from("lead_distribution_configs").delete().eq(
        "organization_id",
        orgId,
      );
      await adminClient.from("facebook_integrations").delete().eq(
        "organization_id",
        orgId,
      );
      await adminClient.from("organization_members").delete().eq(
        "organization_id",
        orgId,
      );
      ...
      for (const uid of memberUserIds) {
        ...
        await adminClient.from("user_section_access").delete().eq(
          "user_id",
          uid,
        );
      }
      ...
      await adminClient.from("subscriptions").delete().eq(
        "user_id",
        target_user_id,
      );
      await adminClient.from("user_section_access").delete().eq(
