...
serve(async (req) => {
  ...
  try {
    ...
    const { instance_name, remoteJid, message_text, leadId, quotedMessageId }:
    ...
    console.log("📤 Enviando mensagem", {
      ...
      messageLength: typeof message_text === "string" ? message_text.length : 0,
    });
    ...
    if (!instance_name || !remoteJid || !message_text || !leadId) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error:
            "Parâmetros obrigatórios: instance_name, remoteJid, message_text, leadId",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 400,
    ...
    // O canal solicitado nunca é substituído silenciosamente. Ele precisa
    // pertencer à organização do lead e estar autorizado para o caller.
    const { data: instanceCheck, error: instanceCheckError } = await supabase
      .from("whatsapp_instances")
      .select("id, status, instance_name, organization_id")
      .eq("instance_name", instance_name)
      .maybeSingle();

    if (instanceCheckError) {
      console.error(
        "❌ Erro ao verificar instância no banco:",
        instanceCheckError,
      );
      return new Response(
        JSON.stringify({
    ...
    if (!instanceCheck) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error:
            "Instância WhatsApp não encontrada. Por favor, reconecte o WhatsApp nas Configurações.",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 404,
    ...
    if (
      !instanceCheck.organization_id ||
      instanceCheck.organization_id !== leadData.organization_id
    ) {
      return new Response(
        JSON.stringify({
          success: false,
    ...
    if (!isInternalCall) {
      const { data: channelAccessOk, error: channelAccessError } =
        await userScopedClient
          .rpc("user_can_access_channel", { p_channel_id: instanceCheck.id });
    ...
    if (instanceCheck.status !== "CONNECTED") {
      console.error("❌ Instância não está conectada:", {
        instance_name: instanceCheck.instance_name,
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error:
            `Instância WhatsApp está ${instanceCheck.status}. Por favor, reconecte o WhatsApp nas Configurações.`,
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 403,
    ...
    const finalInstanceName = instanceCheck.instance_name;
    ...
    const connectionStateUrl =
      `${cleanBaseUrl}/instance/connectionState/${finalInstanceName}`;
    console.log(
      "🔍 Verificando status real na Evolution API:",
      connectionStateUrl,
    );
    ...
    try {
      ...
      if (stateResponse.ok) {
        ...
        if (!isConnectedState(stateData.instance?.state || stateData.state)) {
          ...
          return new Response(
            JSON.stringify({
              success: false,
              error:
                "WhatsApp desconectado. Por favor, reconecte o WhatsApp nas Configurações escaneando o QR Code novamente.",
            }),
            {
              headers: { ...corsHeaders, "Content-Type": "application/json" },
              status: 503,
        ...
        console.warn(
          "⚠️ Não foi possível verificar status da instância, continuando com envio...",
        );
      }
    } catch (statusError) {
    ...
    const sendMessageUrl =
      `${cleanBaseUrl}/message/sendText/${finalInstanceName}`;

    console.log(`🔄 Chamando Evolution API: ${sendMessageUrl}`);
    console.log(
      `📝 Texto da mensagem sendo enviado para Evolution API:`,
      message_text,
    );
      ...
      if (fetchError.name === "AbortError") {
        return new Response(
          JSON.stringify({
            success: false,
            error:
              "Timeout: A Evolution API não respondeu a tempo. Verifique se o serviço está funcionando.",
          }),
          {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
            status: 504,
    ...
    if (!evolutionResponse.ok) {
      ...
      let friendlyError =
        `Evolution API retornou erro ${evolutionResponse.status}`;
      try {
        ...
        if (friendlyError.includes("Connection Closed")) {
          ...
          console.log(
            "🔄 Status da instância atualizado para DISCONNECTED devido a Connection Closed",
          );
        }
      } catch {
      ...
      if (evolutionResponse.status === 404) {
        friendlyError =
          "A instância WhatsApp não existe ou foi desconectada. Por favor, reconecte o WhatsApp nas Configurações.";
    ...
    try {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error:
            "Erro ao processar resposta da Evolution API. A API pode estar offline ou retornando dados inválidos.",
        }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 502,
    ...
    if (leadId) {
      try {
        ...
        if (dbError) {
          console.error(
            "⚠️ Erro ao salvar no banco (mensagem foi enviada):",
            dbError,
          );
        } else {
        ...
        console.error(
          "⚠️ Exceção ao salvar no banco (mensagem foi enviada):",
          dbException,
        );
      }
    }
