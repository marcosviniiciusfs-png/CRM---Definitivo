...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (fbError) {
      ...
      console.log(
        `[CLEANUP-LOGS] Deleted ${fbCount || 0} facebook webhook logs`,
      );
    }
    ...
    if (waError) {
      ...
      console.log(
        `[CLEANUP-LOGS] Deleted ${waCount || 0} whatsapp webhook logs`,
      );
    }
    ...
    if (formError) {
      console.error(
        "[CLEANUP-LOGS] Error deleting form webhook logs:",
        formError,
      );
    } else {
    ...
    if (metaError) {
      console.error(
        "[CLEANUP-LOGS] Error deleting meta conversion logs:",
        metaError,
      );
    } else {
      console.log(
        `[CLEANUP-LOGS] Deleted ${metaCount || 0} meta conversion logs`,
      );
    }

    const totalDeleted = (fbCount || 0) + (waCount || 0) + (formCount || 0) +
      (metaCount || 0);
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message:
          `Cleanup complete. Deleted ${totalDeleted} logs older than 72 hours.`,
        details: {
