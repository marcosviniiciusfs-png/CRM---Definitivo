...
async function syncOneConfig(
  ...
  if (!resp.ok) {
    ...
    await supabase.from("sheet_sync_configs").update({
      ...
      next_sync_at: shouldDeactivate
        ? null
        : new Date(Date.now() + config.sync_interval_minutes * 60_000)
          .toISOString(),
    }).eq("id", config.id);
    ...
    await supabase.from("sheet_sync_logs").insert({
      ...
      error_message: errorMsg +
        (shouldDeactivate ? " — sincronização desativada" : ""),
  ...
  if (allRows.length === 0) {
    await supabase.from("sheet_sync_configs").update({
      ...
      last_error: null,
      error_count: 0,
      next_sync_at: new Date(Date.now() + config.sync_interval_minutes * 60_000)
        .toISOString(),
    }).eq("id", config.id);
    await supabase.from("sheet_sync_logs").insert({
      ...logBase,
  ...
  const findColumnIndex = (crmField: string): number => {
    ...
    if (headers.length) {
      ...
      const idx = headers.findIndex((h) =>
        normalizeKey(String(h)) === normTarget
      );
      if (idx >= 0) return idx;
    }
  ...
  if (phoneIdx < 0 || nomeIdx < 0) {
    await supabase.from("sheet_sync_configs").update({
      last_error:
        "Mapeamento incompleto: faltam colunas Nome ou Telefone (cabeçalho da planilha pode ter mudado).",
  ...
  const existingPhones = new Set(
    (existingLeads || []).map((l: any) => l.telefone_lead),
  );
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    if (config.attribution_strategy === "connector") {
      ...
      config.attribution_strategy === "spreadsheet_column" &&
      config.attribution_column
    ) {
      const colIdx = headers.findIndex((h) =>
        normalizeKey(String(h)) === normalizeKey(config.attribution_column!)
      );
      if (colIdx >= 0) {
...
serve(async (req) => {
  ...
  try {
    if (!isInternalServiceRoleRequest(req)) {
      requireSecretHeader(req, "CRON_SECRET", "x-cron-secret");
    }
    const supabase = createSupabaseAdmin();

    let manualConfigId: string | null = null;
    const body = await readOptionalJsonObject(req, 4 * 1024);
    if (typeof body.config_id === "string" && body.config_id.trim()) {
      manualConfigId = body.config_id.trim();
      if (
        !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i
          .test(manualConfigId)
      ) {
        throw new Error("config_id inválido");
      }
    }
    ...
    let configsQuery = supabase
      ...
      .select(
        "id, organization_id, user_id, spreadsheet_id, sheet_name, header_row, column_map, funnel_id, funnel_stage_id, source_label, attribution_strategy, attribution_column, sync_interval_minutes, error_count",
      )
      .eq("is_active", true)
    ...
    return new Response(
      JSON.stringify({ processed: list.length, succeeded, failed }),
      {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
      },
    ...
    const validationResponse = requestValidationResponse(err, corsHeaders);
    if (validationResponse) return validationResponse;
    console.error("❌ sync-google-sheets falhou:", err);
    return new Response(JSON.stringify({ error: "Falha na sincronização" }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 500,
    });
  }
