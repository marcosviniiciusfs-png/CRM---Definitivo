...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(
      "[CLEANUP] Found columns with auto-delete:",
      columns?.length || 0,
    );
    ...
    for (const column of columns || []) {
      ...
      console.log(
        `[CLEANUP] Processing column "${column.title}" - deleting cards older than ${column.auto_delete_hours}h (threshold: ${hoursAgo.toISOString()})`,
      );
      ...
      if (findError) {
        console.error(
          `[CLEANUP] Error finding old cards in column ${column.id}:`,
          findError,
        );
        continue;
      }
      ...
      console.log(
        `[CLEANUP] Found ${oldCards.length} cards to delete in column "${column.title}"`,
      );
      ...
      if (delError) {
        console.error(
          `[CLEANUP] Error deleting cards in column ${column.id}:`,
          delError,
        );
        continue;
      }
      ...
      if (deleted) {
        ...
        console.log(
          `[CLEANUP] Deleted ${deleted.length} cards from column "${column.title}"`,
        );
      }
    }
